...
def updateWord(word, guesses, letCount):
    for letter in word:
        if letter in guesses:
            ...
            print('_', end=' ')
    return letCount
    #define the function for Menu
def Menu(): #function for scoreboard to be read and opened menu
    print("########################################")
    print("Welcome to Hangman! You will be given 3 categorizes\n and will be asked to guess the word by guessing letters.\nGood Luck!")
    print("#                MENU                  #")
    print("#                                      #")
    print("#       1. Animals                     #")
...
def ExitNow(): #function for ending game and updating the highest score
...
sel = Menu()
print(sel)#this prints my menu
if sel ==4:
        scoreBoard()
        sel=Menu()
if sel ==5:
    ExitNow()
    os._exit #this will exit the game
while sel<=3:
    ...
    while turns > 0 and letCount<wordCount:
        print()
        newguess=input (name + " Give me a letter ")#newguess is variable created that allows you to guess a letter
        ...
        if newguess in word:
            ...
            turns -=1 #the else is when you guess a wrong letter and the number of turns is decreased by one
            print("Sorry, you have ", turns, "turns left")
        letCount=0
        letCount= updateWord(word, guesses, letCount)
    os.system('cls')
    score=3*wordCount+5*turns #this calculates when everytime the usar is finished with the game
    if score > maxScore:
        maxScore=score
    print(maxScore) #maxScore will print the high score
    sel=Menu() #this will take you back to the menu
